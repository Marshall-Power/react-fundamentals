...
I learnt how JSX compiles to the raw React API, the best example is just undernath in the jsx bracket. We can pass props by adding "className='container'", we can pass props literally by adding "{className}" if we have previosly assigned the constant className a string or we can deestructure if we have more than one by adding all props to a single object "const props = { className, children }" and then using the spread operator "{...props}
